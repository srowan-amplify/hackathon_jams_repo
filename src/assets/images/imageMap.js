
const imageMap = {
  background: require('./background.png'),
  startButton: require('./start.png'),
  stopButton: require('./stop.png'),
  // emotions
  surprisedButton: require('./surprised-button.png'),
  happyButton: require('./happy-button.png'),
  curiousButton: require('./curious-button.png'),
  // arrow buttons
  leftButton: require('./left-arrow.png'),
  rightButton: require('./right-arrow.png'),
  // genres
  country: require('./country-button.png'),
  pop: require('./pop-button.png'),
  experimental: require('./experiment-button.png'),
};

export default imageMap;
